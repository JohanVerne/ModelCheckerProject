...
class AlwaysProgressOperator(PropertyOperator):
    ...
    def execute(
        self, composed_state: Tuple[str, Any], action: Any
    ) -> List[Tuple[str, Any]]:
...
def verify_always_progress():
    ...
    def on_entry(composed_state, opaque):
        ...
        safe_actions = [
            a for a in available_actions if progress_predicate(sys_state, a)
        ]
    ...
    print(
        f"Ratio actions safe : {sum(results['safe_ratio'])/len(results['safe_ratio']):.2f}"
    )
